...
**iOS:** By default, downloaded xcframeworks are stored inside `node_modules/{package-name}/.rnrepo-cache/` in a versioned subdirectory keyed by package version and React Native version (e.g. `.rnrepo-cache/0.1.0-rn0.80.0/`). To move the cache outside of `node_modules` (useful for CI or monorepos), set `cacheDir` in `rnrepo.config.json`:
...
Artifacts will be stored under `<cache-dir>/.rnrepo-cache/{package-name}/{version}-rn{rn-version}/` and symlinked back into `node_modules` so CocoaPods can find them. See the [CocoaPods plugin README](packages/build-tools/cocoapods-plugin/README.md) for details.
