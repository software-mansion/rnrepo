...
7. **Expo SDK 55+ with `react-native-worklets` on Android:** `expo-modules-core` links against the `:react-native-worklets` Gradle project directly and hardcodes the path to `libworklets.so` in its CMake configuration, so it cannot be built against a prebuilt worklets AAR in every scenario. In **debug** builds, `react-native-worklets` therefore falls back to sources, and so do the packages that depend on it at the C++ level (`react-native-reanimated`, `@expensify/react-native-live-markdown`) — even if artifacts for your exact versions exist. **Release** builds are unaffected: `expo-modules-core` is compiled from sources against the prebuilt `libworklets.so`, so the hardcoded path resolves and prebuilt worklets are used. To see which of your packages fell back and why, run your build with `--info` and grep for `RNRepo`.
