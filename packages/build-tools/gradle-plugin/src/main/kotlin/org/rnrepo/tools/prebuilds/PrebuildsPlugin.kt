...
class PrebuildsPlugin : Plugin<Project> {
    ...
    private fun configureReactNativeAutolinkingTask(
        project: Project,
        extractTask: org.gradle.api.tasks.TaskProvider<ExtractPrebuiltsTask>,
    ...
    private fun setReactNativeAutolinkingInput(
        ...
        runCatching {
            ...
            if (property == null) {
                logger.warn("Could not configure ${task.name}: React Native task does not expose autolinkInputFile")
                false
            } else {
            ...
            logger.warn("Could not configure ${task.name} to use RNRepo autolinking config: ${error.message}")
            false
        }
}
