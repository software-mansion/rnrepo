...
const HowItWorks: React.FC = () => {
  return (
    <section
      id="how-it-works"
      className="py-16 bg-background border-b border-rnrGrey-80"
    >
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col mb-12">
          <StepItem
            icon={<Icon name="git-branch" className="w-5 h-5" />}
          ...
          <ArrowSeparator />
          <StepItem
            icon={<Icon name="server" className="w-5 h-5" />}
          ...
          <StepItem
            ...
            description={
              <>
                Built artifacts are GPG-signed and published to our public{' '}
                <a
                  href="#"
        ...
        <div className="bg-[#111] border border-rnrGrey-80 p-6 md:p-8 rounded-sm relative overflow-hidden group">
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="max-w-2xl">
              <h3 className="text-2xl font-medium text-rnrGrey-0 mb-4">
                Transparent & Verifiable
              </h3>
              <p className="text-base text-rnrGrey-40 leading-relaxed">
                Every artifact includes a link to its build workflow. You can
                verify the exact commit, build logs, and GPG signature of any
                prebuilt library.
                <br />
                <br />
            ...
            <div className="shrink-0">
              <div className="w-24 h-24 md:w-32 md:h-32 rounded-full border-[8px] border-brandGreen-100 flex items-center justify-center opacity-90">
                <svg
                  className="w-12 h-12 md:w-16 md:h-16 text-brandGreen-100"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="4"
                  viewBox="0 0 24 24"
...
const StepItem: React.FC<{
  ...
  <div className="flex gap-6 items-start">
    <div
      className={`w-12 h-12 border rounded-sm flex items-center justify-center shrink-0 ${colorClass}`}
    >
      <div className="w-5 h-5 flex items-center justify-center">{icon}</div>
    </div>
    <div className="">
      <h3 className="text-lg font-medium text-rnrGrey-0 mb-1">{title}</h3>
      <p className="text-sm text-rnrGrey-40 leading-relaxed max-w-xl">
        {description}
      </p>
    </div>
  </div>
...
const ArrowSeparator: React.FC = () => (
  <div className="flex gap-6">
    <div className="w-12 flex justify-center pt-1 pb-2">
      <Icon name="arrow-down" className="text-rnrGrey-60 w-5 h-5" />
    </div>
  </div>
);
