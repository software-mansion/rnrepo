...
if (!options.setupOnly) {
  const aarPackages = options.packages ? options.packages.split(',').map((p: string) => p.trim()) : [];
  const sourcePackages = options.sourcePackages ? options.sourcePackages.split(',').map((p: string) => p.trim()) : [];
